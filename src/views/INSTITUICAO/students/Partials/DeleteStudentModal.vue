...
const handlePayload = async () => {
  const response = await deleteAluno(alunoToEditInModal.value.id);

  if (response) {
    close.value = false;
    emit("update:open", false);
    emit("update:refresh", true);
    textSuccess.value = "Aluno deletado com Sucesso!";
    ...
    textError.value = "Não foi possivel deletar o Aluno";
  }
};
...
<template>
  <BaseModal :open="close" :closeIcon="true">
    ...
    <template v-slot:body>
      <div class="body" v-if="alunoToEditInModal">
        <p>
          Você tem certeza que deseja desvincular o aluno:
          <span style="font-weight: 600">{{ alunoToEditInModal.nome }}</span
          >?
        </p>
      </div>
    </template>
